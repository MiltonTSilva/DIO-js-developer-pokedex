...
function getInfoSpecies(pokemon) {
  ...
  return fetch(url)
    ...
    .then((data) => {
      ...
      let generationUpper =
        generationSplit[0].charAt(0).toUpperCase() +
        generationSplit[0].substring(1) +
        " " +
        generationSplit[1].toUpperCase();
      ...
      if (
        ...
        if (data.chain.evolves_to[0] != null) {
          if (data.chain.evolves_to[0].evolves_to[0] != null) {
            ...
            pokemon.evolutionsSprites = [
              `${urlSprite}${evolId1[6]}.svg`,
        ...
        data.chain.evolves_to.map((evolv) => {
          pokemon.evolutions +=
            evolv.species.name.charAt(0).toUpperCase() +
            evolv.species.name.substring(1) +
            ", ";
